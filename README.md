...
**Versión:** v0.4.0 (ver [`CHANGELOG.md`](./CHANGELOG.md) y
...
- [`data/catalogo.csv`](./data/catalogo.csv) — catálogo de trabajo real:
  ...
  usuario), esquema propio provisional (ver `ROADMAP.md`). 15 columnas:
  12 del escaneo de Mac + `autor_carpeta`/`autores`/`formato` derivados.
...
- [`scripts/extraer_catalogo.py`](./scripts/extraer_catalogo.py) — genera
  el catálogo y el JSON filtrado a partir de un informe de
  `elyini_scanner.py`.
- [`scripts/enriquecer_catalogo.py`](./scripts/enriquecer_catalogo.py) —
  añade autor(es) y formato al catálogo a partir de ruta/nombre de
  archivo.
